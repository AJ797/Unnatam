...
\paragraph{Total parameter cost.} Per attention layer: $\nhormones \dmodel + \nhormones + 1$ parameters (router, magnitudes, gate). The hormone bank $V$ itself is shared across all attention layers, contributing $\nhormones \dmodel$ non-trainable parameters once. For \textsc{tiny} ($\dmodel = 1024$, $\nhormones = 7$, two attention layers): $2 \times (7 \times 1024 + 7 + 1) = 14{,}352$ trainable parameters total, roughly $0.01\%$ of model parameters.
...
The mechanism is trained in two stages, with the hormone bank extracted from the partially-trained base model in between. The original motivation was to feed the base model's own emerging behavioural axes back into its training signal as an inductive prior on the residual stream; the empirical finding (Section~\ref{sec:experiments}) is that, at the scale we examine, the model develops the routing structure regardless of whether the bank is informative, while the gated injection's effect on the loss is bounded enough to be invisible. The staged schedule remains relevant for a different reason --- it is what allows the mechanism to remain benign --- but its rationale is now empirical rather than presumed.
...
The schedule matters empirically. Section~\ref{sec:experiments} shows that introducing hormone routing as a continuation from a stabilised Stage-$1$ checkpoint is benign --- even with random or fixed-magnitude direction vectors --- while training from scratch with hormones in place from step $0$ (the \textsc{HR-noext} variant) actively degrades performance. We interpret the staged schedule as a precondition for the mechanism's bounded behaviour rather than a free hyperparameter.
